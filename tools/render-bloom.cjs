...
const {Game,render}=require('../experiments/jelly-bloom/engine.js');
...
const g=new Game(917);g.reset();const targets=[60,0,0,0,24,0,0,0,0,0,0,0];let finishAt=null;const taps=[],frames=[];
...
for(let frame=0;frame<fps*50;frame++){
 ...
 if(g.state==='won'&&finishAt===null)finishAt=g.time+1.8;
 if(finishAt&&g.time>=finishAt)break;
}
if(g.state!=='won')throw Error('Replay must reach a real win');
...
const startFrame=30;
const proc=spawn(ffmpeg,['-y','-f','rawvideo','-pix_fmt','rgba','-s',W+'x'+H,'-r',String(fps),'-i','pipe:0','-ss',String(startFrame/fps),'-i',path.join(out,'pops.wav'),'-c:v','libx264','-preset','fast','-crf','20','-pix_fmt','yuv420p','-c:a','aac','-b:a','128k','-movflags','+faststart','-shortest',path.join(out,'jelly-bloom-003.mp4')],{stdio:['pipe','ignore','pipe'],windowsHide:true});let err='';proc.stderr.on('data',b=>{err+=b.toString();if(err.length>8000)err=err.slice(-8000);});proc.stdin.on('error',()=>{});
...
for(let frame=0;frame<total;frame++){
 ...
 ctx.fillStyle='#fcf6eb';ctx.fillRect(0,0,1080,330);
 label('3 PERFECT = GROW BACK',540,195,52);label('JELLY BLOOM',540,268,26,'#9f84bb');
 ...
 const caption=g.state==='won'?'BACK FROM THE BRINK!':g.count<3?'LOST SOME JELLY?':g.count<7?'THREE PERFECT DROPS…':'GROW. STACK. CELEBRATE.';
 ...
 label('K_games · Tiny Play Lab 003',540,1820,29,'#81638f');
...
proc.stdin.end();const [code]=await once(proc,'close');if(code)throw Error(err);console.log(JSON.stringify({duration,frames:total,score:g.count,state:g.state,output:path.join(out,'jelly-bloom-003.mp4')}));})().catch(e=>{console.error(e);proc.kill();process.exitCode=1;});
